...
def teacher_dashboard():
    ...
    menu = ["Rekomendasi", "Konfirmasi Data Siswa", "Lihat Dataset", "Informasi"]  # Hapus "Tambah Data"
    ...
    if choice == "Rekomendasi":
        ...
        st.markdown("<div class='card'><h3>Konfirmasi/Hapus Data Siswa</h3></div>", unsafe_allow_html=True)
        ...
        if pending_data.empty:
            ...
            # Confirmation form
            st.markdown("---")
            st.markdown("<div class='card'><h3>Konfirmasi atau Hapus Data</h3></div>", unsafe_allow_html=True)
            ...
            with st.form("confirmation_form"):
                ...
                # Two buttons: Confirm and Delete
                col1, col2 = st.columns(2)
                with col1:
                    confirm_button = st.form_submit_button("Konfirmasi Tanpa Ubah")
                with col2:
                    delete_button = st.form_submit_button("Hapus Data")
                
                if confirm_button:
                    # Update status tanpa ubah ekstrakurikuler
                    data_handler.update_status(selected_student, "Terkonfirmasi")
                    st.success(f"Data siswa {student_data['Nama']} telah dikonfirmasi!")
                    st.rerun()
                
                if delete_button:
                    # Hapus data siswa
                    data_handler.delete_student(selected_student)
                    st.success(f"Data siswa {student_data['Nama']} telah dihapus!")
                    st.rerun()
